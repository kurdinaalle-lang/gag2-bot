...
def build_weather_post(weather_key: str) -> str:
    label = WEATHER_MAP.get(weather_key, weather_key)
    return (
        "🫯 Special Weather\n\n"
        f"• Weather - {label}\n\n"
        f"⏳ time: {msk_now()} MSK\n🌸 @GrowAGarden2Radar"
...
async def run_loop() -> None:
    ...
    while True:
        try:
            ...
            seeds_changed = (
                rotation_id is not None
                and rotation_id != last_rotation_id
            )

            weather_changed = (
                weather_key is not None
                and weather_key != last_weather_key
            )
